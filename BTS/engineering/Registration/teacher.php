...
$fnameErr = $lnameErr = $addressErr = $emailErr  = $passwordErr = $contactErr= "";  
$fname = $lname = $address = $contact = $email = $password = $image = "";  
...
//String Validation  
    ...
    if (empty($_POST["contact"])) {  
        $contactErr = " Your contact is required";  
    } else {  
...
<div class="form-row">
   ...
   <div class="form-group col-md-12">
    ...
    <input type="text" name="contact" class="form-control" id="inputFathername" placeholder=" Your's contact" autocomplete="off">  
...
<?php  
    if(isset($_POST['submit'])) {  
       ...
       $count=0;
        ...
        $sql="SELECT email from `engineering_teacher`";
        ...
        if($count==0)
        {
          if($fnameErr == "" && $lnameErr == "" && $addressErr == "" && $emailErr == "" && $passwordErr == "" && $contactErr =="" ) {
        
    mysqli_query($db,"INSERT INTO engineering_teacher(firstname,lastname,address,email,contact,password) VALUES('$fname', '$lname', '$address', '$email', $contact, '$password');"); 
